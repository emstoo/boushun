...
The MAC-centered History view is derived on demand from those retained raw snapshots and creates no separate index or stored identity. It shows the beginning of retained or available history, not proof of when a MAC was first used. Each raw snapshot is projected independently with current identity overrides and interface settings. A shared MAC or manual split remains visibly branched; an earlier IP-only observation is not retroactively attached. Missing observations and cache-only records never establish that a device was offline.
