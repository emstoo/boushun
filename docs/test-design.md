...
| ID | Priority | Condition or action | Expected result |
|---|---:|---|---|
| DEP-01 | P0 | Start the standard container and run an API-requested Standard scan | Run non-root with a read-only root filesystem; keep PID 1 effective/permitted capabilities empty and its bounding set limited to NET_RAW; leave `no_new_privs` unset so `/usr/bin/ping` can acquire only its NET_RAW file capability; confirm the server-spawned ping receives the isolated responder's echo |
