...
This document derives required test coverage for Boushun 0.2.0 from product behavior, public APIs, configuration, data model, user interface, and operational constraints, rather than treating existing tests as the specification. The functional scenarios define the required behavior; Section 8 records the current execution paths, CI checks, and validation boundaries.
...
| DEP-12 | P1 | Lint and render the Helm chart with default and representative override values | Produce valid Kubernetes resources whose tag- or digest-selected image, CIDR allowlist, scheduling, persistence, and existing-input mounts follow the supplied values; reject attempts to override chart-managed environment variables |
| DEP-13 | P0 | Render the default Helm workload | Preserve loopback-only HTTP and host-network LAN visibility, run exactly one non-root replica with a read-only root filesystem, restricted `/tmp`, runtime-default seccomp, and only `NET_RAW` added after dropping all capabilities; expose no Service or Ingress |
