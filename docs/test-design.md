...
| DEP-11 | P1 | Publish a version tag through the container workflow | Push one GHCR image index containing `linux/amd64` and `linux/arm64`, with semantic-version, immutable commit, provenance, and SBOM metadata; grant the workflow only repository-read and package-write access |
| DEP-12 | P1 | Lint and render the Helm chart with default and representative override values | Produce valid Kubernetes resources whose image, CIDR allowlist, scheduling, persistence, and existing-input mounts follow the supplied values |
...
| DEP-14 | P0 | Render Helm persistence and Kubernetes API access | Mount a writable `/data` PVC and grant its ServiceAccount only cluster-wide `list` access to core Nodes and Services; permit disabling chart-managed RBAC or using an existing claim without embedding credentials |
