...
This document derives required test coverage for Boushun 0.2.1 from product behavior, public APIs, configuration, data model, user interface, and operational constraints, rather than treating existing tests as the specification. The functional scenarios define the required behavior; Section 8 records the current execution paths, CI checks, and validation boundaries.
...
| ID | Priority | Condition or action | Expected result |
|---|---:|---|---|
| DEP-01 | P0 | Build and start the standard container, audit its privilege-bearing files, and run an API-requested Standard scan | Remove every setuid/setgid bit from the image; retain no file capability except `/usr/bin/ping cap_net_raw=ep`; run non-root with a read-only root filesystem; keep PID 1 effective/permitted capabilities empty and its bounding set limited to NET_RAW; leave `no_new_privs` unset so `/usr/bin/ping` can acquire that capability; confirm the server-spawned ping receives the isolated responder's echo |
...
| DEP-07 | P1 | OUI update receives a valid IEEE MA-L CSV, an HTTP/empty/HTML response, an invalid header, or too few valid records; the loader encounters a missing, unreadable, or invalid file | Require the IEEE header and at least 1,000 valid MA-L records before an atomic 0600 replacement; preserve the existing file on every failure; report connected, missing, unreadable, and invalid loader states distinctly without exposing file content |
...
| DEP-11 | P1 | Publish a version tag through the container workflow | Serialize GHCR publication in one ordered queue; reject commits outside `main` or without every required successful CI check; then push one image index containing `linux/amd64` and `linux/arm64`, with semantic-version, immutable commit, provenance, and SBOM metadata |
| DEP-12 | P1 | Lint, render, and YAML-parse the Helm chart with default and representative override values | Produce valid Kubernetes resources whose tag- or digest-selected image, CIDR allowlist, scheduling, persistence, and existing-input mounts follow the supplied values; every exec probe command element is a string; reject attempts to override chart-managed environment variables |
| DEP-13 | P0 | Render the default Helm workload | Preserve loopback-only HTTP and host-network LAN visibility, run exactly one non-root replica with a read-only root filesystem, restricted `/tmp`, runtime-default seccomp, and only `NET_RAW` added after dropping all capabilities; explicitly allow privilege escalation solely so ping can acquire its image file capability, and expose no Service or Ingress |
| DEP-14 | P0 | Render Helm persistence and Kubernetes API access | Mount a writable `/data` PVC and grant its ServiceAccount only cluster-wide `list` access to core Nodes and Services; namespace-qualify cluster-scoped RBAC names; permit disabling chart-managed RBAC or using an existing claim without embedding credentials |
...
3. `npm run test:helm` lints, renders, and YAML-parses the chart with default and representative override values, then checks its loopback/host-network boundary, single-replica strategy, typed exec probes, security context, persistent storage, minimal Kubernetes inventory RBAC, optional existing claim, and input-volume rendering. `npm run test:container` then builds the production image and validates the production Compose host-network configuration. Before collection, it audits the running image root filesystem and requires zero setuid/setgid files plus exactly one file capability, `/usr/bin/ping cap_net_raw=ep`; this makes a base-image update fail until its privilege-bearing files are reviewed. Runtime checks place the application and one fixture in an internal synthetic network namespace with a separate isolated responder: explicit local configuration loading must discover the fixture interface, an HTTP-requested Standard scan must make the server's PID 1 spawn ping and receive the responder's ICMP echo, and a TCP scan must find the fixture service. PID 1 must have `NoNewPrivs=0`, empty effective/permitted capability sets, and a bounding set containing only NET_RAW, matching the short-term file-capability requirement. The application retains its non-root user, read-only root filesystem, health check, persistent volume, and restricted temporary filesystem. Writes to the root filesystem and execution from `/tmp` must actually fail. Recreating the application container must preserve exported state and layout. After reset, another recreation must preserve the empty state; loading local configuration adds only the probe and configured ranges. The internal bridge has no external route, and fixture/responder processes retain no added capability. The script ignores local overrides and `.env`, refuses an existing `boushun-ci` project or data volume, and removes its synthetic containers, network, and volume on completion or test failure.
