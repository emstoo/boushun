...
export async function startSyntheticDemoServer(options = {}) {
  const dataDirectory = await mkdtemp(path.join(os.tmpdir(), "boushun-browser-demo-"));
  const defaultCollector = async ({ profile } = {}) => {
  ...
  const { server, store } = await createBoushunServer({
    ...
    dataDirectory,
    demo: true,
    allowedCIDRs: options.allowedCIDRs ?? ["192.168.50.0/24"],
    startScheduler: false,
    collector: options.collector ?? defaultCollector,
  });
